        ...
        <div style={{ display: "grid", gridTemplateColumns: "0.9fr 1.1fr", gap: 64, alignItems: "center" }}>
          ...
          <div>
            ...
            <p style={{ color: "var(--on-deep-soft)", fontSize: "0.98rem", lineHeight: 1.7, marginBottom: 32, maxWidth: 46 + "ch" }}>
              Every workflow at Dr. Kassaw Mamma — from intake to diagnosis to discharge — is built like a system: measured, monitored, and continuously verified. Precision isn&apos;t a slogan here, it&apos;s the operating standard.
            </p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "rgba(255,255,255,0.1)", borderRadius: "var(--radius)", overflow: "hidden" }}>
              {specs.map((s, i) => (
                <div key={i} style={{ background: "var(--bg-deep)", padding: "18px 20px" }}>
                  <div className="m-spec-label" style={{ color: "#F2B61D", marginBottom: 8 }}>{s.label}</div>
          ...
          <div style={{ position: "relative" }}>
            <svg viewBox="0 0 520 380" style={{ width: "100%", height: "auto" }} aria-hidden="true">
              ...
              {[[20,60],[20,190],[20,320],[190,90],[160,190],[190,290],[500,60],[500,190],[500,320],[330,90],[360,190],[330,290]].map(([x,y],i) => (
                <circle key={i} cx={x} cy={y} r="3.5" fill="#F2B61D" />
              ))}

              {/* Node labels (left) */}
              <text x="20" y="50" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">INTAKE</text>
              <text x="20" y="180" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">DIAGNOSTICS</text>
              <text x="20" y="310" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">TREATMENT</text>
              {/* Node labels (right) */}
              <text x="410" y="50" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">MONITORING</text>
              <text x="430" y="180" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">RECORDS</text>
              <text x="440" y="310" fill="#F2B61D" fontFamily="var(--font-mono)" fontSize="10" letterSpacing="1">FOLLOW-UP</text>

              {/* Central convergence ring */}
              <circle cx="260" cy="190" r="56" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
              <circle cx="260" cy="190" r="40" fill="var(--primary-900)" stroke="#F2B61D" strokeWidth="1.5" />

              {/* Pulse line inside the node */}
              <path d="M232 190 H246 L253 174 L263 206 L270 182 L276 190 H288" fill="none" stroke="#F2B61D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
